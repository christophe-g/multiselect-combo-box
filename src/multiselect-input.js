 ...
 */
export class MultiselectInput extends MultiselectComboBoxMixin(PatternMixin(InputFieldMixin(ThemableMixin(ElementMixin(PolymerElement))))) {
	static get is() {
	...
	_onChipFocus(e)  {
		...
		console.log('_onChipFocus', e.target);
	}
	_onChipBlur(e)  {
		this._focusedChip = null;
		console.log('_onChipFocus', e.target, this._focusedChip);
	}
	...
	_onKeyDown(e) {
		if (e.key === 'Enter') {
			...
			const item = this.itemLabelPath && this.itemValuePath ? {[this.itemLabelPath]: e.target.value, [this.itemValuePath]: value} : value;
